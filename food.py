"""
food.py
Yılanın yiyeceği normal ve özel (bonus) yemleri temsil eden sınıf.
"""
...
from settings import (
    ...
    SPECIAL_FOOD_CHANCE, SPECIAL_FOOD_DURATION,
    COLOR_FOOD, COLOR_FOOD_OUTLINE,
    COLOR_SPECIAL_FOOD, COLOR_SPECIAL_FOOD_OUTLINE,
)
...
class Food:
    ...
    def respawn(self, occupied_cells):
        """
        Yılanın üzerinde olmadığı rastgele bir hücre seçer.
        Küçük ihtimalle özel (bonus) yem olarak işaretler.
        """
        free_cells = [
            (x, y)
        ...
        if not free_cells:
            # Tahtada boş yer kalmadı (neredeyse imkansız ama güvenlik için)
    ...
    def is_expired(self):
        """Özel yemin süresi dolduysa True döner (normal yem asla süresi dolmaz)."""
        if not self.is_special:
            return False
        return (time.time() - self.spawn_time) > SPECIAL_FOOD_DURATION

    def remaining_ratio(self):
        """Özel yemin ne kadar süresi kaldığını 0.0-1.0 arasında döner (görsel geri sayım için)."""
        if not self.is_special:
            return 1.0
    ...
    def draw(self, surface, sprites=None):
        ...
        if self.is_special:
            ...
            # Zamanla küçülen bir "ömür halkası" çiz
